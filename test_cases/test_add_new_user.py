...
class Test_03_Add_New_Customer:
    ...
    @pytest.mark.regression   #adding markers. Create a pytest.ini file and add marker entries
    @pytest.mark.sanity       #Bcoz pytest doesn't know what regression and sanity markers are
    def test_add_new_customer(self,setup):
        ...
        email = generate_random_email()   #Can't use same email every time the test runs

        self.add_customer.enter_email(email)
        self.add_customer.enter_password("Test@123")
        self.add_customer.enter_firstname("Roshan")
        self.add_customer.enter_lastname("Nepal")
        self.add_customer.select_gender("Male")
        #self.add_customer.enter_dob("11/11/1999")
        self.add_customer.enter_companyname("BookmundiApS")
